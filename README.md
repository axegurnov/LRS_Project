...
* к) public function exceptionSqlAction($value) - Функция исключения sql-запросов из передаваемых данных. В качестве параметра принимает значение, подлежащее проверке на sql-инъекции и возращает профильтрованный результат.
* л) public function redirect($url) - Функция возвращает переход на другую страницу без отправки заголовков.
* 2.1 Контроллеры-потомки содержат либо уникальные действия для каждого случая, либо информацию, необходимую классу Controller для функционирования (например, название таблицы). Также в них происходит подключение моделей и вью, необходимых для конкретного случая.
